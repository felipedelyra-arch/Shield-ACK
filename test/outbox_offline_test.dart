...
class _FakeClient implements FunctionsClient {
  ...
  final Future<Result<Failure, Map<String, dynamic>>> Function(String, Map<String, dynamic>) responder;

  final calls = <({String op, Map<String, dynamic> payload})>[];

  @override
  Future<Result<Failure, Map<String, dynamic>>> call(String name, Map<String, dynamic> payload) {
...
void main() {
  ...
  test('modo avião: item fica pendente e sobe quando a rede volta', () async {
    ...
    final client = _FakeClient((_, __) async =>
        online ? const Ok({'passed': true, 'xpAwarded': 20}) : const Err(OfflineFailure()));

    final outbox = Outbox(db, client);
    final key = await outbox.enqueue('submitQuiz', {'lessonId': 'l1'});

    expect(await outbox.resultFor(key), isNull, reason: 'offline: sem resultado ainda');
  ...
  test('a idempotencyKey vai no payload e é ESTÁVEL entre tentativas', () async {
    var attempts = 0;
    final client = _FakeClient((_, __) async {
      attempts++;
      return attempts < 3 ? const Err(OfflineFailure()) : const Ok({'ok': true});
    });
  ...
  test('falha PERMANENTE não entra em retry — vai direto para dead', () async {
    final client = _FakeClient((_, __) async => const Err(InvalidInputFailure('INVALID_LESSONID')));
    final outbox = Outbox(db, client);
    ...
    expect(client.calls.length, 1, reason: 'payload inválido não deve ser reenviado');
    final row = await db.select(db.outboxItems).getSingle();
    expect(row.status, 'dead');
  });

  test('rate limit respeita o retryAfter do servidor, não o backoff local', () async {
    final client = _FakeClient((_, __) async => const Err(RateLimitedFailure(900)));
    final outbox = Outbox(db, client);
  ...
  test('app encerrado no meio do envio: item inflight é reenviado no boot', () async {
